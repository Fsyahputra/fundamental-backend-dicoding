...
import type { IPlayListService, TJPlaylistSongs, TPLaylist, TPlaylistDTO } from "../types/playlist.js";
...
class PlaylistService implements IPlayListService {
  ...
  findManyPlaylist: (ids: string[]) => Promise<TPLaylist[]> = async (ids: string[]) => {
    if (ids.length === 0) {
      return [];
    }
    const query = {
  ...
  public async save(playList: TPlaylistDTO): Promise<TPLaylist> {
    ...
    const result = await this.pool.query(query);
    return { id: result.rows[0].id, name: result.rows[0].name, owner: result.rows[0].owner };
  }
  ...
  public async savePlaylistSong(playlistId: string, songId: string): Promise<TJPlaylistSongs> {
    const query = {
      text: `INSERT INTO playlist_songs (playlist_id, song_id) VALUES ($1, $2) RETURNING playlist_id, song_id`,
      values: [playlistId, songId],
    };
    const result = await this.pool.query(query);
    return { playlistId: result.rows[0].playlist_id, songId: result.rows[0].song_id };
  }

  public async deletePlaylistSong(playlistId: string, songId: string): Promise<void> {
    const query = {
