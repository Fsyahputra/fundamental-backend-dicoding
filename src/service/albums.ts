...
class AlbumService implements IServiceAlbum {
  ...
  private buildUpdateQuery(album: Partial<AlbumDTO>, id: string): { text: string; values: any[] } {
  ...
  public async save(album: AlbumDTO): Promise<Album> {
    ...
    return { id: result.rows[0].id, name: result.rows[0].name, year: result.rows[0].year };
  }
  ...
  public async update(id: string, album: Partial<AlbumDTO>): Promise<Album | null> {
