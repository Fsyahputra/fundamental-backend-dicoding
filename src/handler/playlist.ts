import type { IPlaylistHandler, IPlayListService, TPlaylistDTO, TPlaylistServiceDependency } from "../types/playlist.js";
import validationObj from "../schema/playlists.js";
import { BadRequestError, ForbiddenError, NotFoundError } from "../exception.js";
...
import type { IActivityService, TActivity, TActivityPresentation } from "../types/activity.js";
...
class PlaylistHandler implements IPlaylistHandler {
  ...
  constructor(playlistDependency: TPlaylistServiceDependency) {
    ...
    this.collaborativePlaylistService = playlistDependency.collaborativePlaylistService;
  ...
  public async postPlaylist(r: R, h: H): Promise<Lf.ReturnValue> {
    ...
    const cred = r.auth.credentials.user as TAuthObj;
    const ownerId = cred.id;
    const playlistData: TPlaylistDTO = {
      name,
      owner: ownerId,
    };
    const { error } = this.validator.postSchema.validate(playlistData);
    if (error) {
      throw new BadRequestError(`Invalid playlist data: ${error.message}`);
    }
  ...
  public async getPlaylist(r: R, h: H): Promise<Lf.ReturnValue> {
    ...
    const collaborativePlaylists = await this.collaborativePlaylistService.getPlaylistIdByUserId(cred.id);
    ...
    const playlists = await this.playlistService.findManyPlaylist(userPlaylists);
    const ownerIds = playlists.map((playlist) => playlist.owner);
    const accounts = await this.userService.getManyByIds(ownerIds);
    const accountUsernames = new Map(accounts.map((account) => [account.id, account.username]));
  ...
  public async postSongToPlaylist(r: R, h: H): Promise<Lf.ReturnValue> {
    ...
    const collaborarId = (await this.collaborativePlaylistService.getUserIdByPlaylistId(playlistId)) ?? [];
    if (!playlist) {
      throw new BadRequestError(`Playlist with id ${playlistId} not found`);
    }

    if (playlist.owner !== id && !collaborarId.includes(id)) {
      throw new ForbiddenError(`You do not have permission to add songs to this playlist`);
    }
  ...
  public async getSongsByPlaylistId(r: R, h: H): Promise<Lf.ReturnValue> {
    const playlistId = r.params["id"];
    const playlist = await this.playlistService.getById(playlistId);
    const creds = r.auth.credentials.user as TAuthObj;
    const id = creds.id;
    ...
    const playlistSongs = await this.musicService.getSongsInPlaylist(playlistId);
  ...
  public async deleteSongFromPlaylistId(r: R, h: H): Promise<Lf.ReturnValue> {
    ...
    await this.activityService.addActivity({
      userId: (r.auth.credentials.user as TAuthObj).id,
      playlistId,
      songId,
      action: "delete",
    });
  ...
  private async prepareActivityPresentation(activity: TActivity, r: R): Promise<TActivityPresentation> {
  ...
  public async getPlaylistActivity(r: R, h: H): Promise<Lf.ReturnValue> {
    const playlistId = r.params["id"];
    const activities = await this.activityService.getActivitiesByPlaylistId(playlistId);
    const activityPresentations = await Promise.all(activities.map((activity) => this.prepareActivityPresentation(activity, r)));
    if (activityPresentations.length === 0) {
      throw new NotFoundError(`No activities found for playlist with id ${playlistId}`);
    }
